...
React + TypeScript, shadcn/ui + Tailwind. Feature-folder structure mirroring the backend slices. **TanStack Query** manages server state — availability is volatile server state (refetched, near-real-time), and this avoids hand-rolling cache logic that could show stale (already-taken) slots. **react-i18next** owns translation (pt-BR / en); the API returns error codes + params, never translated prose. Accessibility target WCAG 2.1 AA (Radix primitives under shadcn help by default).
