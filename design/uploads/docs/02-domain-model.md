...
- **Cancellation / reschedule (F3):** the patient may cancel or reschedule their own appointment up to a **configurable cutoff (default 24 h)** before start; inside the cutoff, only the front desk can. This is the concrete demonstration of RBAC + ownership-based authorization coexisting — the patient has power over their own data, bounded by a temporal business rule.
...
- **I1** — `start < end`; duration equals the configured `ProfessionalAppointmentType.durationMinutes`.
...
- **I7** — within the professional's working hours and not intersecting any `TimeBlock` at booking time.
...
|---|---|---|
| I4 / I5 / I6 (hard overlap) | **Database** | `EXCLUDE USING gist` on `tstzrange` with `btree_gist` (`professional_id WITH =`, `time_range WITH &&`), **partial** `WHERE status = 'Scheduled'` so cancelled/rescheduled rows free the slot |
| I1–I3, I7, I8 (business rules) | **Domain** | Validated in the aggregate before persistence; friendly errors |
| Appointment vs. **external** block overlap | **Reconciliation** | Not a DB constraint (different tables; external block arrives later) → `ReconciliationConflict(Open)` + front-desk signal |
