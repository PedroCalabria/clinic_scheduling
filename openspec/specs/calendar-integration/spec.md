...
The clinic's authorization to a professional's **own** external calendar: how it is obtained,
how the long-lived credential that results is protected, what states the authorization can be
in and how each is observed, and how it is given up. This is the one place the system reaches
outside its own database, and the one place it holds a credential belonging to somebody else.

The product reason is the failure this whole project exists to prevent. A professional blocks a
personal commitment in the calendar they actually look at; the front desk cannot see it, books a
patient into that time, and the clinic finds out on the day. Closing that needs the appointments
to reach their calendar and their blocks to reach ours — and both need permission first.

**Permission is a separate moment from signing in**, deliberately. Signing in asks for identity
scopes only; the calendar scope is requested when a professional presses connect, over the
identity grant already held. Folding the two together would ask every patient for calendar
access they will never use, on the consent screen they are least willing to think about.

Two properties do most of the work. **The credential is encrypted at rest** under a key that is
not in the repository — the first secret in this system for which that is true. And **a status
is an observation, not a fact**: with nothing calling the provider on a schedule, what a screen
can honestly say is what was true when it last looked, and when that was.

Established by change `calendar-connection` (6a), which delivers the connection and deliberately
propagates nothing. Outbound sync — the outbox, the dispatcher, event idempotency — arrives with
`calendar-outbound` (6b); inbound sync, external time blocks, and the reconciliation queue with
`calendar-inbound` (7).
