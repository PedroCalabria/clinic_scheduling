...
The **professional-configuration** half — a professional's specialties, their per-type
durations, and their working hours — extends this same capability in change 3b, along with the
clinic timezone that converting wall-clock working hours to instants requires.

Established by change `clinic-catalog` (change 3a of the build order), following
`identity-session` from change 2, whose administrator role policy every endpoint here carries.
